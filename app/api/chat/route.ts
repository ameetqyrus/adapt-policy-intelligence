...
const allowedModels = new Set(['gpt-5.6-luna', 'gpt-5.6-terra', 'gpt-5.6-sol', 'gpt-5.4-mini']);

function errorMessage(status: number) {
  if (status === 401) return 'OpenAI could not authenticate this key. Check the key and try again.';
  if (status === 403) return 'This OpenAI project does not have access to the selected model or feature.';
  if (status === 429) return 'The OpenAI project reached a rate or spending limit. Check its usage settings and try again.';
  ...
  for (const item of output as Array<Record<string, unknown>>) {
    ...
    for (const content of item.content as Array<Record<string, unknown>>) {
      ...
      for (const annotation of content.annotations as Array<Record<string, unknown>>) {
        ...
        citations.set(annotation.url, { title: typeof annotation.title === 'string' ? annotation.title : annotation.url, url: annotation.url });
      }
    }
  }
  if (!textParts.length && typeof payload.output_text === 'string') textParts.push(payload.output_text);
...
export async function POST(request: Request) {
  ...
  let body: { model?: string; message?: string; history?: ChatTurn[]; context?: unknown; research?: boolean };
  try {
    const raw = await request.text();
    if (raw.length > 60_000) return Response.json({ error: 'The chat request is too large.' }, { status: 413 });
    body = JSON.parse(raw);
  } catch {
    return Response.json({ error: 'The chat request could not be read.' }, { status: 400 });
  }

  const model = body.model ?? 'gpt-5.6-luna';
  const message = body.message?.trim() ?? '';
  if (!allowedModels.has(model)) return Response.json({ error: 'Choose a supported OpenAI model.' }, { status: 400 });
  if (!message || message.length > 4_000) return Response.json({ error: 'Enter a follow-up question under 4,000 characters.' }, { status: 400 });

  const history = (Array.isArray(body.history) ? body.history : [])
    .filter((turn): turn is ChatTurn => (turn?.role === 'user' || turn?.role === 'assistant') && typeof turn.content === 'string')
    .slice(-12)
    .map((turn) => ({ role: turn.role, content: turn.content.slice(0, 4_000) }));
...
${JSON.stringify(body.context ?? {})}`;
  ...
  try {
    ...
    return Response.json({ error: 'The OpenAI request timed out. Please try again.' }, { status: 504 });
  }

  if (!upstream.ok) return Response.json({ error: errorMessage(upstream.status) }, { status: upstream.status });
  const responsePayload = await upstream.json() as Record<string, unknown>;
  const result = collectOutput(responsePayload);
  if (!result.text) return Response.json({ error: 'OpenAI returned no readable answer. Please try again.' }, { status: 502 });
