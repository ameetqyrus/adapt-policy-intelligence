...
function buildAnswer(home: County, peer: County, prompt: string) {
  ...
  const emphasis = /avoid|mistake/.test(lower)
    ...
    : /education|workforce|spend/.test(lower)
  ...
  return {
    title: peerAhead ? `${compactName(peer.name)} has stronger current wage or employment outcomes, but the explanation is not a single spending number.` : 'The peer comparison is mixed; neither county is uniformly stronger across ADAPT’s indicators.',
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch('/data/county-context.json')
      ...
      .catch(() => { /* The exemplar remains usable if the full catalog is unavailable. */ });
  }, []);
  ...
  const answer = useMemo(() => buildAnswer(home, peer, asked), [home, peer, asked]);
  ...
  function ask(event: FormEvent) { event.preventDefault(); if (prompt.trim()) setAsked(prompt.trim()); }

  return <main className="app-shell">
    <aside className="sidebar">
      <div className="brand"><span>A</span><div><strong>AdapT</strong><small>County intelligence</small></div></div>
      <button className="new-question" type="button" onClick={() => setPrompt('')}>＋ New question</button>
      <nav><p>Workspace</p><a className="active" href="#comparison">⌁ County comparison</a><a href="#evidence">▤ Evidence <span>{sources.length}</span></a><a href="#method">◎ Method</a></nav>
    ...
    <section className="workspace" id="comparison">
      <header className="topbar"><div><small>AMERICAN DREAM ACHIEVABILITY PROGRESS TRACKER</small><h1>Peer-county policy intelligence</h1></div><span className="verified">● ADAPT context loaded</span></header>
      <div className="content-grid">
        <section className="conversation">
          ...
          <div className="question"><span>AD</span><div><small>YOUR QUESTION</small><p>{asked}</p></div></div>
          <article className="answer-card">
            ...
            <div className="recommendation"><strong>Recommended next step</strong><p>Compare programme eligibility, delivery partners, cost per participant and measured employment retention before recommending transfer to {compactName(home.name)}.</p></div>
          </article>
          <div className="suggestions">{questions.map((question) => <button type="button" key={question} onClick={() => setPrompt(question)}>{question} →</button>)}</div>
          <form className="composer" onSubmit={ask}><textarea aria-label="Ask a county policy question" rows={2} value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder={`Ask about ${compactName(home.name)} and ${compactName(peer.name)}…`} /><button type="submit" aria-label="Send question">↑</button></form>
        </section>
